...
function normalizeShowtimeToMinute(input) {
    ...
    if(isNaN(d.getTime())){
        try {d = new Date(decodeURIComponent(String(input))); }
        catch { d = new Date(String(input)); }
    }
...
//this function gives total amount and it also calculate amount for standard and recliner
...
export async function createBooking(req, res) {
   ...
   // const conflictingSeats = seatIdList.filter(s => occupiedSeats.has(s));
    ...
    const doc = {
        ...
        showtime,
        auditorium,
        duration: movieSnapshot.durationMins,
        seats: normalizedSeats,
        bestPrice: movie?.seatPrices?.standard ?? movie?.price ?? 0,
        amount: totalMain,
    ...
    if(paymentMethod === 'card'){
        ...
        try {
            ...
            return res.status(500).json({
                success: false,
                message: "Payment not configured", error: err.message
            });
        }
...
export async function getOccupiedSeat(req, res) {
    try {
        const {movieId, movieName, showtime: showtimeRaw, audi: audiRaw } = req.query;
        if(!showtimeRaw) return res.status(400).json({
            success: false, message: ' showtime query is required'
        });

        const auditorium = String(audiRaw || req.query.auditorium || 'Audi 1');
        let parsed;
        try {parsed = normalizeShowtimeToMinute(showtimeRaw); } catch {
            return res.status(400).json({
                success: false,
        ...
        const q = {showtime: {$gte: start, $lt: end }, auditorium, status: {$in: BOOKING_STATUSES }};
        const movieClauses = buildMovieMatchClause(movieId, movieName);
