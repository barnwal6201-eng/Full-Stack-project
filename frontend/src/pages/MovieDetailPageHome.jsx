...
import { ArrowLeft, Calendar, Clock, Play, Star, User, Users, X } from 'lucide-react'
...
const MovieDetailPageHome = () => {
    ...
    const showTimeDays = useMemo(() => {
        ...
        (movie.slots || []).forEach((raw) => {
            ...
            const dataKey = formatDateKey(d, TZ);
            if(!slotsByDate[dataKey]) slotsByDate[dataKey] = [];
            slotsByDate[dataKey].push({iso, audi});
        });
    ...
    /**
     * Get booked count for specific datetime and audi (iif availble).
     ...
     */
    const getBookedCounterFor = (datetime, audi = "Audi 1") => {
        try {
          ...
          <div className={movieDetailHStyles.posterContainer}>
            <div className={movieDetailHStyles.posterCard}>
                <div className={movieDetailHStyles.posterImageContainer} style={{maxWidth: "320px"}}>
                    ...
                    loading='lazy' className={movieDetailHStyles.posterImage}
                    onError={(e) => {
            ...
            <div className={movieDetailHStyles.showtimesCard}>
                ...
                <div className={movieDetailHStyles.showtimesGrid}>
                    {showTimeDays[selectedDay]?.showTimes && showTimeDays[selectedDay].showTimes.length ? (
                        showTimeDays[selectedDay].showTimes.map((showtime, index) => {
                            ...
                            const bookedCount = getBookedCounterFor(showtime.datetime, showtime.audi);
                ...
                {selectedTime && (
                    <div className={movieDetailHStyles.bookNowContainer}>
                        <button onClick={handleBookNow} className={movieDetailHStyles.bookNowButton} aria-label='Proceed'>
                            ...
                            <div key={idx} className={movieDetailHStyles.castMember}>
                                <div className={movieDetailHStyles.castImageContainer}>
                                    {c.img ? (
                                        ...
                                        loading='lazy' className={movieDetailHStyles.castImage}
                                        onError={(e) => {
                                        e.currentTarget.onerror = null;
                                         e.currentTarget.src = 
                                ...
                                {directors.length ? (
                                    directors.slice(0, 2).map((d, i) => (
                                        <div key={i} className='flex flex-col items-center'>
                                            {d?.img ?(
                                                <img src={d.img} alt={d.name || `Director ${i+1}`}
                                                loading='lazy' className={movieDetailHStyles.crewImage}
                                                onError={(e) => {
                                                e.currentTarget.onerror = null;
                                                e.currentTarget.src = 
